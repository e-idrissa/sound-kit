...
const APP_URL = process.env.NEXT_PUBLIC_APP_URL || 'http://warehousehost:3000';
    ...
    await transporter.sendMail({
      ...
      html: `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #e5e7eb; border-radius: 8px;">
          ...
          <p style="font-size: 16px; line-height: 1.5;">An administrator has created an account for you in our instruments management system. To get started, you'll need to first login with the following credentials.</p>
          
          <div style="background-color: #f9fafb; border-left: 4px solid #2563eb; padding: 15px; margin: 20px 0;">
            <p style="margin: 0; font-size: 14px;"><strong>Account Credentials:</strong></p>
...
export const sendResetPasswordEmail = async (
  ...
  try {
    const resetPasswordUrl = `${APP_URL}/forgot-password/reset?token=${token}`;

    await transporter.sendMail({
      from: EMAIL_FROM,
      to: email,
      subject: 'Reset Password - SoundKit',
      html: `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #e5e7eb; border-radius: 8px;">
          <div style="text-align: center; margin-bottom: 20px;">
            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="#2563eb" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-clapperboard-icon lucide-clapperboard">
          ...
          <p style="font-size: 16px; line-height: 1.5;">We have recievied a request to reset your password. Please click on the link below to reset your password.</p>
          <p style="font-size: 16px; line-height: 1.5;"><a href="${resetPasswordUrl}" style="color: #2563eb; text-decoration: none;">🔗 Reset Password Link</a></p>
          ...
          <div style="margin-top: 30px; padding-top: 20px; border-top: 1px solid #e5e7eb;">
            <p style="font-size: 14px; color: #6b7280; margin-bottom: 5px;">Please note:</p>
            <ul style="font-size: 14px; color: #6b7280; padding-left: 20px; margin-top: 5px;">
              <li>This reset password link will expire in 1 hour.</li>
              <li>Keep your credentials safe and do not share them with anyone.</li>
              <li>If you did not expect this email, please contact your administrator.</li>
            </ul>
          </div>
    ...
    return {success: true, url: resetPasswordUrl};
  } catch (error) {
    console.error('Error sending email:', error);
